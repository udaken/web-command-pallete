...
Web Command Palette is a lightweight browser extension that brings a VS Code-style command palette to your browsing experience. Define custom commands using XPath or CSS Selectors to click buttons, focus inputs, or navigate links—all without reaching for your mouse.

## ✨ Features

- **Universal Command Palette:** Works on any website defined in your settings.
- **Customizable Shortcuts:** Default is `Ctrl+Alt+P`. Fully configurable in settings.
- **Powerful Definitions:** Use XPath or CSS Selectors to target any element.
- **Built-in Debug Tools:**
  - **Toggle Info:** Check matched rules, current settings, and URL status directly in the palette.
  - **Copy SITEINFO Template:** Instantly generate a JSON template for the current site to simplify configuration.
- **Flexible Configuration:** Load rules from local JSON or subscribe to external URLs (e.g., GitHub Gists).
- **Security First:** `click` actions are safely demoted to `focus` on untrusted sites.
...
1.  **Open the Palette:** Press `Ctrl+Alt+P` (default) on a supported page.
...
2.  Select **Debug: Copy SITEINFO Template**.
3.  Paste the template into the "Custom SITEINFO" box in **Options** and add your commands.
...
### 1. General Settings
- **Keyboard Shortcut:** Click the input and press your desired key combination.
...
**Tip:** A complete **JSON Schema** is available at the bottom of the Options page to help you write valid configurations.
